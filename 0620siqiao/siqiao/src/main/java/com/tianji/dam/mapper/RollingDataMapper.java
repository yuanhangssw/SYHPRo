...
public interface RollingDataMapper {
    ...
    @Select("select VehicleID,CoordX,CoordY,CoordLX,CoordLY,CoordRX,CoordRY,BeforeCoordLX,BeforeCoordLY,BeforeCoordRX,BeforeCoordRY from `${tableName}` ORDER BY `Timestamp`")
    public List<RollingData> getAllRollingDatasimple(@Param("tableName") String tableName);
