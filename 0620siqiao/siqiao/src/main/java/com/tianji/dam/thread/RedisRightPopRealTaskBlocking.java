...
public class RedisRightPopRealTaskBlocking extends Observable implements Runnable {
    ...
    private static final long TWO_HOURS_IN_MILLIS = 2 * 60 * 60 * 1000L;
    public static final Map<String, Long> lastWarningTimes = new ConcurrentHashMap<>();
    ...
    /*
    速度预警   该车 每两小时触发  新增 speedcnumber
     */
    public void speedwarning(RollingData rollingData, double currentspeed, double normalspeed, Integer cangid, String cangname, Car car) {
        ...
        Long lastWarningTime = lastWarningTimes.get(car.getRemark());  //

        if (GlobCache.speedWarningNumber.get(car.getRemark()) == null) {
            GlobCache.speedWarningNumber.put(car.getRemark(), 1L);
        } else {
            Long speedcnumber = GlobCache.speedWarningNumber.get(car.getRemark());
            speedcnumber++;
            GlobCache.speedWarningNumber.put(car.getRemark(), speedcnumber);

        }
        if (GlobCache.speedWarningNumber.get(car.getRemark()) > 10) {
            if (lastWarningTime == null || (currentTime - lastWarningTime) >= TWO_HOURS_IN_MILLIS) {
                for (TWarningUser user : users) {
                    if (null != user.getTel() && !"".equals(user.getTel())) {
                        savesendrecord(currentspeed, normalspeed, cangid, car, user, cangname);
                        ...
                        System.out.println("我发送预警短信了");
                    }
                }
                lastWarningTimes.put(car.getRemark(), currentTime);
                GlobCache.speedWarningNumber.put(car.getRemark(), 0L);
                System.out.println("速度预警 时间和次数重置！！！");
     ...
     * @param time
     */
    private void savesendrecord(double currentspeed, double normalspeed, Integer cangid, Car car, TWarningUser user, String cangname) {
        ...
        record.setTitle(cangname);
        record.setCreateTime(new Date());
        record.setFreedom1("1");
        smsrecordMapper.insertSmsSendRecord(record);
    }


    private void savesendrecord2(double currentspeed, double normalspeed, Integer cangid, Car car, TWarningUser user, String cangname) {
        SmsSendRecord record = new SmsSendRecord();
        record.setCarName(car.getRemark());
        record.setCurrentValue(new BigDecimal(currentspeed).setScale(2, RoundingMode.HALF_UP));
        record.setNormalValue(new BigDecimal(normalspeed).setScale(2, RoundingMode.HALF_UP));
