<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="项目" align="center" prop="unitName">
        <template slot-scope="scope">
          {{ projectName }}
        </template>
      </el-table-column>
      <el-table-column label="巡查单元" align="center" prop="unitName">
        <template slot-scope="scope">
          {{ unitName }}
        </template>
      </el-table-column>
      <el-table-column label="巡查类型" align="center" prop="fillType">
        <template slot-scope="scope">
          {{ scope.row.fillType == 1 ? "白蚁" : "獾狐鼠等" }}
        </template>
      </el-table-column>
      <el-table-column label="渗漏处数" align="center" prop="leaksNumber" />
      <el-table-column label="穿坝处数" align="center" prop="throughNumber" />
      <el-table-column
        label="跌窝处数"
        align="center"
        prop="dropSocketNumber"
      />
      <el-table-column label="挖巢数量(个)" align="center" prop="nestDigging" />
      <el-table-column label="施药面积(㎡)" align="center" prop="chargeArea" />
      <el-table-column
        label="灌浆量(延米)"
        align="center"
        prop="groutingQuantity"
      />
      <el-table-column
        label="投入资金(万元)"
        align="center"
      ...
      <el-table-column
        label="治理数量"
        align="center"
        prop="quantityGovernance"
      />

      <!-- <el-table-column label="巡查用户" align="center" prop="inspector" /> -->
      <!-- <el-table-column
        label="操作"
        align="center"
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleopenpatrol(scope.row)"
...
<script>
import { listPatrol, listFill } from "@/api/system/unit";
import { getPatrolDetail } from "@/api/system/audit";
export default {
  name: "Audit",
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFill(this.queryParams).then((response) => {
