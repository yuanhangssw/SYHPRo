    ...
    >
      ...
      >
        ...
        <view class="text-area">
          <view class="paroect-picker">未登录 点击进入登录页面</view>
          <!--轮播-->
  ...
  methods: {
    ...
    getProjectList() {
      ...
      getProjects(this.userId).then((res) => {
        ...
        // this.$store.state.user.projectId = this.projectList[0].value;
        // uni.setStorageSync("projectId", this.projectList[0].value);
        // this.$store.state.user.projectName = this.projectList[0].text;
        this.getInfo();
      });
    },
    // 选择项目
    change(e) {
      // this.$store.state.user.projectId = value;
      // uni.setStorageSync("projectId", value);
      // let project = this.projectList.find((item) => item.value === value);
      // this.$store.state.user.projectName = project.text;
