 ...
 */
@Slf4j
public class CalculateGridForZhuangHistory implements Callable<Integer> {
    ...
    public CalculateGridForZhuangHistory() {
    }
                    ...
                    if (dis < TrackConstant.MIN_DIS) {
                        ...
                        ps4.y =  ld2[1];
                                ...
                                if (count == 4) {
                                    ...
                                    datatime=   rollingData02.getTimestamp();
                                    carid =     rollingData02.getVehicleID();
                                    cev =       rollingData02.getCurrentEvolution();
                                    ev  =       rollingData02.getElevation();
                                    speed =     rollingData02.getSpeed();
                                    ac =        rollingData02.getAcceleration();
                                    fre =       rollingData02.getFrequency();
                                    vib  =      rollingData02.getVibrateValue();
                                    isvib =    rollingData02.getVibrateValue() >=50;
                                        ...
                                        try {
                                            ...
                                            if (n >= 0 && m >= 0 && n < height && m < width) {
                                                MatrixItem item = matrix[m][n];
                                                    item.getTimestampList().add(datatime);
                                                    item.getVehicleIDList().add(carid);
                                                    item.getCurrentEvolution().add(cev);
                                                    item.setRollingTimes(item.getRollingTimes() + 1);
                                                    item.getElevationList().add(ev);
                                                    item.getSpeedList().add(speed);
                                                    item.getAccelerationList().add(ac);
                                                    item.getFrequencyList().add(fre);
                                                    item.getVibrateValueList().add(vib);
                                                    if (isvib) {
                                                        //动碾遍数+1
